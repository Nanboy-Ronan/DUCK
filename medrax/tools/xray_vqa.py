...
class XRayVQATool(BaseTool):
    ...
    @staticmethod
    def _apply_transformers_cache_compat() -> None:
        """Backfill removed cache APIs expected by CheXagent remote code."""
        try:
        ...
        if not hasattr(DynamicCache, "get_max_length"):
            DynamicCache.get_max_length = DynamicCache.get_max_cache_shape  # type: ignore[attr-defined]

        if not hasattr(DynamicCache, "get_usable_length"):
            DynamicCache.get_usable_length = (  # type: ignore[attr-defined]
                lambda self, new_seq_length=0, layer_idx=0: self.get_seq_length(layer_idx)
            )
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._apply_transformers_cache_compat()
    ...
    def _generate_response(self, image_paths: List[str], prompt: str, max_new_tokens: int) -> str:
        ...
        with torch.inference_mode():
            try:
                ...
                # Some CheXagent + transformers combinations hit KV-cache shape drift.
                # Retry once with cache disabled to keep the tool usable.
